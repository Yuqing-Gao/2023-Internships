...
def item_analysis(df):
    ...
    # model percentage
    # by region and brand
    grouped1 = df.groupby(["REGION2", "BRAND", "Item"])["COPIES"].value_counts().unstack(fill_value=0)
    ...
    high_modeled_percentage_brands2 = grouped1_filtered[grouped1_filtered["MODELED_percentage"] > grouped1_filtered["MODELED_percentage"].mean()]
    high_modeled_percentage_brands2.to_excel("MODELED percentage by region & brand & item.xlsx")

    # by country channel
    grouped3 = df.groupby(["CountryChannel", "BRAND", "Item"])["COPIES"].value_counts().unstack(fill_value=0)
    grouped3["MODELED_percentage"] = grouped3["MODELED"] / grouped3.sum(axis=1)
    high_modeled_percentage_brands_bychannel = grouped3[grouped3["MODELED_percentage"] >= 0.5]  # mean ~= 0.25, median = 0, 样本极不平衡
    high_modeled_percentage_brands_bychannel.to_excel("MODELED percentage by channel & brand & item.xlsx")

    # difference (by region and brand)
    df1 = df[["REGION2", "BRAND", "Item", "CountryChannel", "COPIES", "Sales Units E,NC", "Sales Units"]]
    df1 = df1.set_index(["REGION2", "BRAND", "Item"])
    ...
    grouped5 = df2.groupby(["REGION2", "BRAND", "Item"])["difference"].sum().reset_index()
    grouped5 = grouped5.set_index(["REGION2", "BRAND", "Item"])
    ...
    high_difference_brands2.to_excel("High difference by region & brand & item (Aug.).xlsx")

    # difference (by channel and brand)
    df1 = df[["REGION2", "BRAND", "Item", "CountryChannel", "COPIES", "Sales Units E,NC", "Sales Units"]]
    df1 = df1.set_index(["CountryChannel", "BRAND", "Item"])
    df1["difference"] = abs(df1["Sales Units E,NC"] - df1["Sales Units"])
    df2 = df1[["Sales Units E,NC", "Sales Units", "difference"]]
    df2 = df2.fillna(0)  # fill nan as 0
    grouped6 = df2.groupby(["CountryChannel", "BRAND", "Item"])["difference"].sum().reset_index()
    grouped6 = grouped6.set_index(["CountryChannel", "BRAND", "Item"])
    high_difference_channels = grouped6[grouped6["difference"] > grouped6["difference"].mean()]
    high_difference_channels.to_excel("High difference by channel & brand & item (Aug.).xlsx")

    """
    九月
    # difference (by region and brand)
    df1 = df[["REGION2", "BRAND", "Item", "CountryChannel", "COPIES", "Sales Units E,NC.1", "Sales Units.1"]]
    df1 = df1.set_index(["REGION2", "BRAND", "Item"])
    ...
    high_difference_brands2 = grouped5_filtered[
        grouped5_filtered["difference"] > grouped5_filtered["difference"].mean()]
    high_difference_brands2.to_excel("High difference by region & brand & item (Sep.).xlsx")
    ...
    grouped6 = grouped6.set_index(["CountryChannel", "BRAND", "Item"])
    high_difference_channels = grouped6[grouped6["difference"] > grouped6["difference"].mean()]
    high_difference_channels.to_excel("High difference by channel & brand & item (Sep.).xlsx")
    """
